...
- [ ] Re-correr este agente sobre la rama pasa sin bloqueantes.
...
- [ ] Re-correr este agente completo y verificar que esta task ya no aparece
...
- **Bloqueantes presentes** → `/apply` en cada sub-issue para resolverlos. Re-correr este agente cuando todas las tasks Critical/High estén cerradas.
...
- Este agente es solo para **pre-deploy**. Para revisión profunda de código OWASP, usar el agente `audit`. Para barrida completa del proyecto, usar el agente `pentest`.
- En hotfixes críticos, este agente puede correrse en modo rápido (solo pasos 1, 2, 3, 8) pero nunca saltarse completamente.
...
- **No ejecutar** este agente dentro de `/apply` o `/build`.

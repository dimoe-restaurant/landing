...
- agente `secure` → mira el **próximo deploy** (rápido, bloqueante)
- este agente (`audit`) → mira el **PR actual** (profundo en lo que cambió)
- agente `pentest` → mira **todo el proyecto** (exhaustivo, periódico)

## Modelo de trabajo

Igual que el agente `pentest`, pero acotado a los cambios del PR:
...
| A06 | Vulnerable Components | Cubierto por el agente `secure` |
...
- **Sin hallazgos** → continuar con `/build` o el agente `pr-review` y luego al flujo de deploy.

## Notas

- Este agente no ejecuta nada destructivo — solo lee, reporta y crea issues.
...
- **No ejecutar** este agente dentro de `/apply` o `/build` — interrumpiría el flujo.

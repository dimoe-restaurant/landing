...
> Nombrado así (no `/plan`) porque `plan` choca con el plan mode nativo
> de Claude Code — un skill custom con el mismo nombre lo sobreescribe
> en silencio, sin aviso ni indicio en `/help`.
...
**Regla dura:** si el dev pide editar código sin tener un work-item asignado y `in-progress`, no se edita — primero pasa por `/plan-task`. Ver regla `no-edit-without-plan` en `CLAUDE.md`.
...
**Default = 1 work-item.** Por defecto, una invocación de `/plan-task` produce **un solo work-item**. Esto es lo normal y lo que más se usa. Tener muchas tasks dentro **no es razón para partir** — un work-item con 8-10 tasks coherentes está bien y sigue siendo un único PR limpio al final.
...
Si no hay overlap → seguir al paso 2 sin avisar nada (no contaminar el output con un "✓ Sin overlap" en cada `/plan-task`).
...
### 3.5 Criterios de aceptación por tipo de trabajo

Usar estos patrones como base — adaptar al trabajo concreto.

**Componente / página Next.js nueva:**
```markdown
- [ ] Componente `<Name>.tsx` en `components/<area>/` o `app/<ruta>/page.tsx`
- [ ] Props tipadas con `interface` explícita; no `any`
- [ ] Responsive: móvil (375px), tablet (768px), desktop (1280px)
- [ ] Accesible: roles ARIA correctos, contraste AA, navegable con teclado
- [ ] `pnpm build` verde (sin errores TypeScript)
- [ ] Spec Playwright: happy path + caso de error/estado vacío
```

**Integración con API / servicio externo:**
```markdown
- [ ] Variables de entorno nuevas en `.env.example` con comentario
- [ ] Credenciales cargadas desde `process.env`, nunca hardcodeadas
- [ ] Manejo de errores: timeout, 4xx, 5xx no rompen la UI
- [ ] `pnpm build` verde
- [ ] Spec Playwright: mockear la API con MSW si es posible
```

**Refactor / chore técnico:**
```markdown
- [ ] Comportamiento observable idéntico antes y después
- [ ] `pnpm build` + `pnpm lint` verdes
- [ ] Tests Playwright existentes siguen pasando sin modificaciones
```

**Configuración / infra (CI, Vercel, tooling):**
```markdown
- [ ] Cambio documentado en `CLAUDE.md` o en el ADR correspondiente si es decisión arquitectural
- [ ] Variables de entorno nuevas en `.env.example`
- [ ] Deploy preview de Vercel funciona tras el merge a `dev`
```

### 3.9 End-to-end completeness check (obligatorio antes de confirmar)

**Un plan con promesas sin dueño no se confirma — se ajusta o se agregan issues.**

Antes de mostrar la propuesta al dev para confirmación, auditar cada work-item y task por cabos sueltos:

| Tipo de cabo suelto | Cómo detectarlo | Cómo resolver |
|---|---|---|
| Ruta frontend referenciada | `href=`, `router.push(`, URL literal en el plan | Verificar que la ruta existe en `app/*/page.tsx`. Si no: issue hermano o eliminar la mención. |
| Endpoint / servicio externo nuevo | La UI lo llama pero no hay task que lo implemente | Agregar task o issue hermano explícito. |
| Variable de entorno nueva | El plan la menciona pero no hay task de configurarla | Agregar subtarea en la task correspondiente o task dedicada. |
| "Out of scope", "follow-up", "pendiente", "más adelante" | Buscar literalmente esas palabras en el borrador | Cada ítem así debe tener su propio issue — sin excepciones. Crear issue hermano en este mismo lote o eliminar la mención. |
| Feature flag nueva | El plan la menciona sin tracking | AC explícito con nombre del flag y su default. |

El plan se puede confirmar **sólo si** cada promesa cumple una de estas condiciones:
- 🔥 Está cubierta por un AC del mismo issue.
- 🔥 Está cubierta por otro issue del mismo lote.
- 🔥 Existe un issue previo abierto o cerrado que la cubre (linkear como dependencia).
- 🔥 Se crea un issue hermano en este mismo lote para trackearlo.

Si un cabo suelto no cae en ninguna de las 4 → **ajustar el plan** (agregar el issue o eliminar la promesa) antes de mostrar al dev.
...
¿Arrancamos algún work-item ahora?
  ...
  3. No, dejar el lote planificado y arrancar después con /session-start + /apply
```
...
Si el dev elige 3 → el lote queda listo. Cualquier dev puede tomar uno con `/session-start` + `/apply` después.
...
**Regla:** si lo que descubriste es parte de cerrar BIEN lo que estás construyendo → nueva task en el mismo padre. Si es algo que ya estaba mergeado en producción y necesita su propio ciclo → nuevo work-item de tipo `fix` (puede planearse con un nuevo `/plan-task`).

## Buenas prácticas

- **Toda planificación = uno o varios work-items padre.** No hay tasks huérfanas.
- **Default 1 work-item por `/plan-task`.** Solo crear varios cuando los alcances son **temáticamente inconexos** entre sí. Tener muchas tasks coherentes no es razón para partir.
...
- **`/plan-task` no crea código ni ramas.** Solo issues, sub-issues y links al Project. Las ramas se crean cuando `/apply` arranca un work-item.
...
- **Dev no quiere arrancar aún** → el lote queda listo. La próxima vez `/session-start` lo lista junto con cualquier otro abierto.
- **Plan afecta múltiples repos** → agente `cross` para coordinar cambios cross-repo

...
Después de `/plan-task` (si el dev eligió arrancar un work-item del lote) o desde `/session-start` (si el dev eligió continuar/empezar un work-item ya planificado). Es el comando que convierte el plan en código real, una task a la vez.

**Regla `no-edit-without-plan`:** si el dev pide editar código y NO hay un work-item asignado a él en `in-progress` con tasks abiertas, `/apply` **rechaza la edición** y redirige a `/plan-task`. Ver paso 0.5.
...
Opciones:
  1. Planificar uno nuevo  → /plan-task
  2. Tomar un work-item asignado sin empezar (si lo hay) → /session-start
  3. Cancelar
```

**Solo se sale del guardrail si:**
- El dev acaba de invocar `/plan-task` y eligió "arrancar #N" (entonces `/apply` recibe `#N` como argumento explícito).
- El dev acaba de invocar `/session-start` y eligió "continuar/empezar #N".
...
Tras esto, cualquier `/session-start` de otro dev verá el work-item como tomado y no lo ofrecerá.
...
Opciones:
  ...
  2. Replantificar esta task para evitar esos archivos (/plan-task)
...
Si descubres que la task se solapa con otra → parar, comentarlo en el work-item, y dejar que el dev decida (puede que toque replanificar con `/plan-task`).
...
¿Planificar el chore con /plan-task? (sí/no — no bloquea esta task)
```

Si el dev dice no, continuar sin gate. Si dice sí, dejar la nota en el output y delegar a `/plan-task` después de cerrar la task actual.
...
- Volver a correr.
- Si tras 3 intentos sigue fallando → invocar `/investigate-bug` con el error específico.
...
3. **El nuevo trabajo NO se mete en esta rama.** Volver a `dev` (`git checkout dev && git pull --ff-only`) y arrancar el nuevo work-item desde ahí, en su propia rama `<tipo>/<N>-<slug>`. Si no hay work-item para el nuevo trabajo → primero `/plan-task`, después `/apply`.
...
- **Si forma parte de cerrar BIEN el work-item activo** → crear una nueva task hija del mismo work-item padre (ver sección "Agregar tasks durante el desarrollo" en `/plan-task`).
- **Si es un problema de algo ya mergeado en producción** → es un nuevo work-item de tipo `fix`, no parte de este. Comunicárselo al dev (puede planearse con `/plan-task` después).

## Siguiente paso

- **Task implementada y tests OK** → `/build` (commit + push + cerrar task; al ser la última del work-item, ofrecerá PR). Opcionalmente `/self-review` antes, para una pasada rápida de bugs/simplificación sobre el diff.
- **Tests fallan después de 3 intentos** → `/investigate-bug` con el error específico
- **Detectas que otro issue ya resolvió parte** → `/sync` para reconciliar
- **La task resultó más grande de lo planeado** → `/plan-task` para dividirla en más tasks
...
- **Crear la rama del work-item es responsabilidad de `/apply`,** no de `/plan-task`. La primera invocación sobre un work-item nuevo es la que la crea desde `dev`.

...
- Antes de empezar una nueva tarea (después de `/session-start`) si el repo tiene actividad reciente de otros.
...
- **Backlog limpio, puedes seguir con tu tarea** → `/apply` o `/plan-task`
- **Detectaste solapamiento en archivos** → comunicar con el otro dev antes de `/apply`
- **Muchos issues desactualizados** → agente `triage` para limpieza profunda en bulk
- **Hay issues bloqueados** → `/plan-task` para resolver dependencias primero
...
- Correr `/sync` antes de cada `/plan-task` en equipos de más de una persona.

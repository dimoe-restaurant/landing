...
**Requisito previo:** el agente `secure` debe haber pasado sin bloqueantes. Si no lo has corrido, este skill lo ejecuta primero.
...
### 0.7. Ejecutar el agente secure si no se ha corrido

```bash
# Si no hay evidencia reciente del agente secure, ejecutarlo ahora
# El deploy NO continúa si el agente secure encuentra bloqueantes
```
...
- [ ] Agente \`secure\` re-corrido pasa sin bloqueantes.
...
- **Error en el pipeline de CI** → `/investigate-bug` con los logs del workflow.

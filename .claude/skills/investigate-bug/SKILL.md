...
> Nombrado así (no `/debug`) porque `debug` choca con un bundled skill
> nativo de Claude Code — el reemplazo es 100% silencioso (sin aviso en
> `/help` ni al ejecutar), así que evitar el nombre es la única forma
> real de prevenir la confusión.
...
```bash
gh issue comment <N> --body "### Fix aplicado — /investigate-bug
...
- **Fix en hotfix para producción rota** → agente `secure` (verificar) → `/deploy` urgente
- **Causa raíz no encontrada tras 3 intentos** → escalar: documentar en el issue y pedir ayuda humana
- **Fix requiere cambios en otros repos** → agente `cross` para coordinar

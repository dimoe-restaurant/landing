...
El fix sigue el flujo: `/apply` → `/test` → agente `secure` → `/deploy`
...
- **Servicio estable de nuevo** → `/investigate-bug` en rama `hotfix/*` para encontrar causa raíz
- **Causa raíz identificada** → `/apply` → `/test` → agente `secure` → `/deploy` (flujo urgente)

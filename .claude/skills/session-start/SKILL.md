...
> Nombrado así (no `/init`) porque `init` choca con el slash command
> nativo de Claude Code que genera un CLAUDE.md escaneando el repo — un
> skill custom con el mismo nombre lo sobreescribe en silencio, sin
> aviso ni indicio en `/help`.

Inicia una sesión de desarrollo. Orienta a Claude sobre el estado actual del proyecto.

## Modo de invocación

- **Sin argumentos** → Full Mode: orientación completa, estado del repo, menú de work-items.
- **Con número de issue** (`/session-start #12`, `/session-start 12`) → Focused Mode: salta la orientación y va directo al work-item indicado.
...
— es la base de trabajo compartida entre features, y las skills siguientes (`/plan-task`,
...
Esa decisión **solo dura la sesión actual**. El próximo `/session-start` volverá a `dev`.

Si `dev` no existe en el remote → invocar `/branches` para crearla y reiniciar `/session-start`.
...
Antes de mostrar el menú, verificar que la base de código no está rota:

```bash
pnpm build 2>&1 | tail -8
```

Si el build falla → **reportarlo antes de cualquier otra cosa** y pedir al dev que lo resuelva. No empezar trabajo nuevo sobre un repo roto. Mostrar el error y sugerir `/investigate-bug`.

Si el build pasa → continuar silenciosamente (no mostrar "✓ build OK" — es ruido).
...
Drift responde *"¿quedé desactualizado?"*. Overlap responde *"¿alguien más está tocando los mismos archivos que yo?"* — son preguntas distintas y ambas importan en repos donde varios chats trabajan en paralelo.
...
Si no hay overlap, no mostrar nada (no contaminar el output de `/session-start` con un "✓").
...
No buscar más allá de esto. Si más adelante una skill detecta otra inconsistencia puntual, la arregla en el momento; no es trabajo de `/session-start` recorrer todo el backlog cada vez.
...
# Por defecto NO se muestran en /session-start para no abrumar.
```
...
**Si todo vacío** → mensaje corto: "No hay work-items asignados ni libres. ¿Planificar algo nuevo? → `/plan-task`".
...
### 4. Focused Mode — si se pasó un número de issue

Si el dev invocó `/session-start #N` o `/session-start N`, ejecutar este modo en lugar del paso 5.

```bash
source .claude/scripts/gh-isolated.sh || exit 1

# Leer issue + todos sus comentarios
gh issue view <N> --json number,title,body,assignees,labels,state,url,comments
```

Verificar:
1. **¿El issue existe y está abierto?** Si no existe o está cerrado → avisar y ofrecer `/plan-task`.
2. **¿Tiene sub-issues (tasks) abiertas?** Cargarlas:
   ```bash
   gh api graphql -f query='...' # mismo query que paso 2
   ```
3. **¿Hay rama del work-item ya creada?**
   ```bash
   git branch --list "<tipo>/$N-*" && git ls-remote --heads origin "<tipo>/$N-*"
   ```

Presentar directo al dev:

```
=== Focused Mode — work-item #12 ===
...
Rama: feature/12-sistema-pagos-stripe (existe, al día con dev)

¿Arrancamos con /apply en la task #42? [S/n]
```

Si el dev confirma → delegar a `/apply` con `#N`.
Si el work-item no tiene tasks → sugerir `/plan-task` para crear las tasks primero.

**Saltar pasos 5–6 completamente en Focused Mode.**
...
¿Qué quieres hacer?
  ...
  4. Planificar algo nuevo → /plan-task
  5. Otra cosa
```
...
- **Opción 4:** invocar `/plan-task` directamente.
- **Opción 5:** escuchar al dev — pero **no editar código sin work-item**. Si pide tocar archivos sin que haya un work-item activo y tasks abiertas, redirigir a `/plan-task` (regla `no-edit-without-plan`).

### 6.5. Guardrail no-edit-without-plan (mid-chat)

Si en cualquier momento de la sesión, **después de `/session-start`**, el dev pide modificar código (ej: "agrega un endpoint para X", "arregla el bug de Y") **y no hay un work-item asignado al dev en `in-progress` con tasks abiertas** que cubra ese trabajo:
...
Opciones:
  1. Planificar primero  → /plan-task (recomendado, conversacional)
  2. Si es un fix urgente → /plan-task modo express (un work-item con una sola task,
...
- Trabajo de auditoría/lectura (agentes `audit`, `pentest`, `pr-review`) — no escriben código de producción.
...
- **Planificar algo nuevo** → `/plan-task`
- **Otros devs hicieron cambios recientes** → `/sync` primero, luego volver aquí
- **Hay un PR abierto esperando review** → agente `pr-review`
- **No existe rama `dev` en el repo** → `/branches` para normalizar antes de continuar

## Notas

- Si no hay work-items asignados ni libres, sugerir revisar el backlog del GitHub Project o invocar `/plan-task`.
...
- **Regla `no-edit-without-plan` (paso 6.5):** mid-chat, si el dev pide editar código sin work-item de respaldo, redirigir a `/plan-task`. No improvisar ediciones en `dev` ni en rama de feature sin task que lo cubra.
- **Visibilidad multi-dev:** el lote del plan puede tener piezas tomadas por otros. `/session-start` lista propios + libres. Para tomar uno libre, `/apply` se encarga de marcarlo "tomado" para que el resto del equipo lo vea.

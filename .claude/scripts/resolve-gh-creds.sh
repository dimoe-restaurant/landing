...
# 4. gh auth token — sesión activa de gh, no-interactivo. Se prueba ANTES que el
#    credential store del sistema porque `git credential fill` puede disparar un
#    prompt gráfico (GCM/browser login) en Windows/macOS y colgar la sesión.
...
# 5. git credential fill (keychain/store del sistema) — ÚLTIMO RECURSO.
#    GIT_TERMINAL_PROMPT=0 evita que cuelgue esperando un prompt interactivo;
#    si el helper configurado igual abre una ventana gráfica (GCM), falla rápido
#    en vez de bloquear la sesión indefinidamente.
_rgc_try_credential_fill() {
  local hint_user="$1"
  local filled user pass
  if [ -n "$hint_user" ]; then
    filled=$(GIT_TERMINAL_PROMPT=0 printf 'protocol=https\nhost=github.com\nusername=%s\n\n' "$hint_user" | GIT_TERMINAL_PROMPT=0 timeout 5 git credential fill 2>/dev/null)
  else
